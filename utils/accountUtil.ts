...
export const ACCOUNT_SORT: Record<string, (u1: Account, u2: Account) => number> = {
  email: (u1: Account, u2: Account) => (u1.email && u2.email ? u1.email.localeCompare(u2.email) : u1.email ? -1 : 0),
  firstName: (u1: Account, u2: Account) =>
    u1.firstName && u2.firstName ? u1.firstName.localeCompare(u2.firstName) : u1.firstName ? -1 : 0,
  lastName: (u1: Account, u2: Account) =>
    u1.lastName && u2.lastName ? u1.lastName.localeCompare(u2.lastName) : u1.lastName ? -1 : 0,
  locale: (u1: Account, u2: Account) =>
    u1.locale && u2.locale ? u1.locale.localeCompare(u2.locale) : u1.locale ? -1 : 0,
...
export const sortByField = (array: Account[], field: string, ascending: boolean) =>
  ascending ? array.sort(ACCOUNT_SORT[field]) : array.sort(ACCOUNT_SORT[field]).reverse();
