...
<script setup lang="ts">
  ...
  const emit = defineEmits<{
    added: [obj: SourceType];
    addCanceled: [];
    edited: [obj: SourceType];
    editCanceled: [obj: SourceType];
    deleted: [obj: SourceType];
  }>();
  ...
  const onAddSubmitted = async (obj: MobilettoOrmObject) => {
    ...
    successSnackbar.value = parseMessage(props.msgAddSuccess, messages.value, {
        type: messages.value.typename_source,
        id: SourceTypeDef.id(obj),
    });
    emit("added", obj);
    addObject.value = {} as SourceType;
    return await sourceStore.search();
  }
  watch(createSourceServerErrors, () => {
    if (createSourceServerErrors.value && Object.keys(createSourceServerErrors.value).length > 0) {
      errorSnackbar.value = parseMessage(props.msgAddError, messages.value, {
        type: messages.value.typename_source,
        error: JSON.stringify(createSourceServerErrors),
      });
    }
  });
  ...
  watch(editSourceServerErrors, () => {
    if (editSourceServerErrors.value && Object.keys(editSourceServerErrors.value).length > 0) {
      errorSnackbar.value = parseMessage(props.msgEditError, messages.value, {
        type: messages.value.typename_source,
        id: SourceTypeDef.id(editingObject.value),
        error: JSON.stringify(editSourceServerErrors),
      });
    }
  });
  ...
  const delObject = (obj: MobilettoOrmObject) => {
      ...
      sourceStore.delete(id, deleteSourceServerErrors)
          .then(() => {
              ...
              emit("deleted", obj);
              return sourceStore.search();
          });
  };
  watch(deleteSourceServerErrors, () => {
    if (deleteSourceServerErrors.value && Object.keys(deleteSourceServerErrors.value).length > 0) {
        errorSnackbar.value = parseMessage(props.msgDeleteError, messages.value, {
            type: messages.value.typename_source,
            id: deletingObject.value ? SourceTypeDef.id(deletingObject.value) : "null",
            error: JSON.stringify(deleteSourceServerErrors),
        });
    }
  });
