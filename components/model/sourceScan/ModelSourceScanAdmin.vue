...
<template>
  <v-container>
    ...
    <v-row>
      <v-col>
        <h2>{{ adminTitle() }}</h2>
        <b v-if="messageExists('title_sourceScan_admin_details', messages)">
          {{ messages.title_sourceScan_admin_details }}
        </b>
      </v-col>
    </v-row>
    <v-row v-if="addingObject">
      <v-col>
        <OrmForm
          v-if="allRefsLoaded() && sourceScanTypeDef"
          form-name="add_sourceScan_form"
          :type-def="sourceScanTypeDef"
          :type-name-message="typeNameMessage"
          :thing="addObject"
          :fields="sourceScanTypeDefFields"
          :create="true"
          :read-only-object="() => false"
          :server-errors="createSourceScanServerErrors"
    ...
    <v-row v-else-if="Object.keys(editingObject).length > 0">
      <v-col>
        <OrmForm
          v-if="allRefsLoaded() && sourceScanTypeDef"
          form-name="edit_sourceScan_form"
          :type-def="sourceScanTypeDef"
          type-name-message="typeNameMessage"
          :thing="editingObject"
          :fields="sourceScanTypeDefFields"
          :create="false"
          :read-only-object="() => false"
          :server-errors="editSourceScanServerErrors"
          :label-prefixes="labelPfx"
          :hint-suffixes="['_description']"
    ...
    <v-row v-else-if="allRefsLoaded() && sourceScanTypeDef">
      <v-col>
        <v-container>
          <v-form @submit.prevent="searchObjects">
            <v-row v-if="(sourceScanList && sourceScanList.length > 0) || searched">
              <v-col>
                  <v-text-field
                    v-model="searchTerms"
                    :label="messages.label_search"
                    :disabled="sourceScanStore.sourceScanBusy"
                    type="text"
                    name="searchTerms"
                    class="form-control"
            ...
            <v-row>
              <v-col>
                <v-btn class="btn btn-primary" :disabled="sourceScanStore.sourceScanBusy" @click.stop="searchObjects">
          ...
          <v-row>
            <v-col>
              <table v-if="sourceScanList && sourceScanList.length > 0">
                <thead>
                <tr>
                ...
                <tbody>
                <tr v-for="(obj, objIndex) in sourceScanList" :key="objIndex">
                  <td v-for="(fieldName, fieldIndex) in tableFields" :key="fieldIndex">
                  <OrmFieldDisplay v-if="sourceScanTypeDef.fields[fieldName] || fieldName.startsWith('_meta')" :field="fieldName.startsWith('_meta') ? metaField(fieldName) : sourceScanTypeDef.fields[fieldName]" :value="deepGet(obj, fieldName)" />
                  </td>
                  ...
                  <NuxtLink
                    ...
                    :to="{ path: `${actionConfig(action).path}/${deepGet(obj, sourceScanTypeDef.idField(obj) as string)}` }"
                    >
                      <v-btn>
                  ...
                  <td>
                    <v-btn v-if="canEdit(obj, sourceScanList)" :disabled="sourceScanStore.sourceScanBusy" @click.stop="showEditOrm(obj)">
                      <Icon name="material-symbols:edit" />
                    </v-btn>
                  </td>
                  <td>
                    <v-btn v-if="canDelete(obj, sourceScanList)" :disabled="sourceScanStore.sourceScanBusy" @click.stop="delObject(obj)">
          ...
          <v-row v-if="canAdd()">
            <v-col>
              <v-btn class="btn btn-primary" :disabled="sourceScanStore.sourceScanBusy" @click.stop="showAddOrm">
...
<script setup lang="ts">
  ...
  const props = withDefaults(
    defineProps<{
      ...
      typeNameMessage: () => "typename_sourceScan",
  ...
  const emit = defineEmits<{
    added: [obj: SourceScanType];
    addCanceled: [];
    edited: [obj: SourceScanType];
    editCanceled: [obj: SourceScanType];
    deleted: [obj: SourceScanType];
  }>();

  const labelPfx: Ref<string[]> = ref(["admin_label_sourceScan_", "label_", ""]);
  ...
  const adminTitle = () => messageExists("admin_title_sourceScan_administration", messages.value)
    ? messages.value.admin_title_sourceScan_administration
  ...
  const addObject = ref({} as SourceScanType);
  const createSourceScanServerErrors = ref({} as MobilettoOrmValidationErrors);

  const editingObject = ref({} as SourceScanType);
  const editSourceScanServerErrors = ref({} as MobilettoOrmValidationErrors);
  const deleteSourceScanServerErrors = ref({} as MobilettoOrmValidationErrors);
  ...
  const sourceScanStore = useSourceScanStore();
  const { sourceScanList  } = storeToRefs(sourceScanStore);
  const sourceScanTypeDef: Ref<MobilettoOrmTypeDef | null> = ref(null);
  const sourceScanTypeDefFields: Ref<MobilettoOrmFieldDefConfig[] | undefined> = ref(undefined);
  const searchQuery = () => {
  ...
  const onAddSubmitted = async (obj: MobilettoOrmObject) => {
    await sourceScanStore
      .create(obj as SourceScanType, createSourceScanServerErrors);
    addingObject.value = false;
    successSnackbar.value = parseMessage(props.msgAddSuccess, messages.value, {
        type: messages.value.typename_sourceScan,
        id: SourceScanTypeDef.id(obj),
    });
    emit("added", obj);
    addObject.value = {} as SourceScanType;
    return await sourceScanStore.search();
  }
  watch(createSourceScanServerErrors, () => {
    if (createSourceScanServerErrors.value && Object.keys(createSourceScanServerErrors.value).length > 0) {
      errorSnackbar.value = parseMessage(props.msgAddError, messages.value, {
        type: messages.value.typename_sourceScan,
        error: JSON.stringify(createSourceScanServerErrors),
      });
    }
  });
  ...
  const onEditSubmitted = async (obj: MobilettoOrmObject) => {
    await sourceScanStore
      .update(obj as SourceScanType, editSourceScanServerErrors);
    editingObject.value = {} as SourceScanType;
  ...
  watch(editSourceScanServerErrors, () => {
    if (editSourceScanServerErrors.value && Object.keys(editSourceScanServerErrors.value).length > 0) {
      errorSnackbar.value = parseMessage(props.msgEditError, messages.value, {
        type: messages.value.typename_sourceScan,
        id: SourceScanTypeDef.id(editingObject.value),
        error: JSON.stringify(editSourceScanServerErrors),
      });
    }
  });
  ...
  const showEditOrm = (obj: MobilettoOrmObject) => {
    ...
    if (obj) {
      const id = SourceScanTypeDef.id(obj);
      if (id && id.length > 0) {
        addingObject.value = false; // ensure add form is closed
        editingObject.value = JSON.parse(JSON.stringify(obj)) as SourceScanType;
      }
    }
  };
  const onEditCancel = () => {
  ...
  const delObject = (obj: MobilettoOrmObject) => {
      ...
      const id = SourceScanTypeDef.id(obj);
      sourceScanStore.delete(id, deleteSourceScanServerErrors)
          .then(() => {
              ...
              emit("deleted", obj);
              return sourceScanStore.search();
          });
  };
  watch(deleteSourceScanServerErrors, () => {
    if (deleteSourceScanServerErrors.value && Object.keys(deleteSourceScanServerErrors.value).length > 0) {
        errorSnackbar.value = parseMessage(props.msgDeleteError, messages.value, {
            type: messages.value.typename_sourceScan,
            id: deletingObject.value ? SourceScanTypeDef.id(deletingObject.value) : "null",
            error: JSON.stringify(deleteSourceScanServerErrors),
        });
    }
  });
  watch(sourceScanList, (newList) => {
    if (newList && Array.isArray(newList) && newList.length === 0 && searchTerms.value && searchTerms.value.length === 0) {
      if (navigating.value) return;
      navigating.value = true;
      navigateTo("/admin/sourceScan/setup");
    }
  });
