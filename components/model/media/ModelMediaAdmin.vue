...
<template>
  <v-container>
    ...
    <v-row>
      <v-col>
        <h2>{{ adminTitle() }}</h2>
        <b v-if="messageExists('admin_title_media_administration_details', messages)">
          {{ messages.admin_title_media_administration_details }}
        </b>
      </v-col>
    </v-row>
                  ...
                  <td v-if="Object.keys(actionConfigs).length > 0">
                    <div v-for="(action, actionIndex) in Object.keys(actionConfigs)" :key="actionIndex">
                      <div v-if="actionEnabled(obj, action) && actionConfig(action).path">
                        <NuxtLink
                          :to="{ path: `${actionConfig(action).path.replace(/\[id]/g, deepGet(obj, mediaTypeDef.idField(obj) as string))}` }"
                          >
