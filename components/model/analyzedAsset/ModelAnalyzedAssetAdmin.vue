...
<template>
  <v-container>
    ...
    <v-row>
      <v-col>
        <h2>{{ adminTitle() }}</h2>
        <b v-if="messageExists('title_analyzedAsset_admin_details', messages)">
          {{ messages.title_analyzedAsset_admin_details }}
        </b>
      </v-col>
    </v-row>
    <div>
      <v-row v-if="addingObject">
        <v-col>
          <OrmForm
            v-if="allRefsLoaded() && analyzedAssetTypeDef"
            form-name="add_analyzedAsset_form"
            :type-def="analyzedAssetTypeDef"
            :type-name-message="typeNameMessage"
            :thing="addObject"
            :fields="analyzedAssetTypeDefFields"
            :create="true"
            :read-only-object="() => false"
            :server-errors="createAnalyzedAssetServerErrors"
      ...
      <v-row v-else-if="Object.keys(editingObject).length > 0">
        <v-col>
          <OrmForm
            v-if="allRefsLoaded() && analyzedAssetTypeDef"
            form-name="edit_analyzedAsset_form"
            :type-def="analyzedAssetTypeDef"
            type-name-message="typeNameMessage"
            :thing="editingObject"
            :fields="analyzedAssetTypeDefFields"
            :create="false"
            :read-only-object="() => false"
            :server-errors="editAnalyzedAssetServerErrors"
            :label-prefixes="labelPfx"
            :hint-suffixes="['_description']"
      ...
      <v-row v-else-if="allRefsLoaded() && analyzedAssetTypeDef">
        <v-col>
          <v-container>
            <v-row v-if="(analyzedAssetList && analyzedAssetList.length > 0) || searched">
              <v-col>
                <div>
                  <v-form @submit.prevent="searchObjects">
                    <div class="form-group">
                      <v-text-field
                        ...
                        :disabled="analyzedAssetStore.analyzedAssetBusy"
                        type="text"
                        name="searchTerms"
                        class="form-control"
                        @keyup.enter="searchObjects"
                      />
                      <v-btn class="btn btn-primary" :disabled="analyzedAssetStore.analyzedAssetBusy" @click.stop="searchObjects">
            ...
            <v-row>
              <v-col>
                <table v-if="analyzedAssetList && analyzedAssetList.length > 0">
                  <thead>
                  <tr>
                  ...
                  <tbody>
                  <tr v-for="(obj, objIndex) in analyzedAssetList" :key="objIndex">
                    <td v-for="(fieldName, fieldIndex) in tableFields" :key="fieldIndex">
                      <OrmFieldDisplay v-if="analyzedAssetTypeDef.fields[fieldName] || fieldName.startsWith('_meta')" :field="fieldName.startsWith('_meta') ? metaField(fieldName) : analyzedAssetTypeDef.fields[fieldName]" :value="deepGet(obj, fieldName)" />
                    </td>
                    <td v-if="Object.keys(actionConfigs).length > 0">
                      <div v-for="(action, actionIndex) in Object.keys(actionConfigs)" :key="actionIndex">
                        <NuxtLink
                          ...
                          :to="{ path: `${actionConfig(action).path}/${deepGet(obj, analyzedAssetTypeDef.idField(obj) as string)}` }"
                        >
                          <v-btn>
                    ...
                    <td>
                      <v-btn v-if="canEdit(obj, analyzedAssetList)" :disabled="analyzedAssetStore.analyzedAssetBusy" @click.stop="showEditOrm(obj)">
                        <Icon name="material-symbols:edit" />
                      </v-btn>
                    </td>
                    <td>
                      <v-btn v-if="canDelete(obj, analyzedAssetList)" :disabled="analyzedAssetStore.analyzedAssetBusy" @click.stop="delObject(obj)">
            ...
            <v-row>
              <v-col>
                <v-btn class="btn btn-primary" :disabled="analyzedAssetStore.analyzedAssetBusy" @click.stop="showAddOrm">
...
<script setup lang="ts">
  ...
  import { useAnalyzedAssetStore } from "~/stores/model/analyzedAssetStore";
  ...
  const props = withDefaults(
    defineProps<{
      ...
      typeNameMessage: () => "typename_analyzedAsset",
  ...
  const emit = defineEmits<{
    added: [obj: AnalyzedAssetType];
    addCanceled: [];
    edited: [obj: AnalyzedAssetType];
    editCanceled: [obj: AnalyzedAssetType];
    deleted: [obj: AnalyzedAssetType];
  }>();

  const labelPfx: Ref<string[]> = ref(["admin_label_analyzedAsset_", "label_", ""]);
  ...
  const adminTitle = () => messageExists("admin_title_analyzedAsset_administration", messages.value)
    ? messages.value.admin_title_analyzedAsset_administration
  ...
  const addObject = ref({} as AnalyzedAssetType);
  const createAnalyzedAssetServerErrors = ref({} as MobilettoOrmValidationErrors);

  const editingObject = ref({} as AnalyzedAssetType);
  const editAnalyzedAssetServerErrors = ref({} as MobilettoOrmValidationErrors);
  const deleteAnalyzedAssetServerErrors = ref({} as MobilettoOrmValidationErrors);
  ...
  const analyzedAssetStore = useAnalyzedAssetStore();
  const { analyzedAssetList  } = storeToRefs(analyzedAssetStore);
  const analyzedAssetTypeDef: Ref<MobilettoOrmTypeDef | null> = ref(null);
  const analyzedAssetTypeDefFields: Ref<MobilettoOrmFieldDefConfig[] | undefined> = ref(undefined);
  ...
  const searchObjects = () => {
    ...
    if (lastQuery.value && JSON.stringify(lastQuery.value) === JSON.stringify(query)) {
      ...
      analyzedAssetStore.search(query);
    }
  };

  const navigating = ref(false);
  const initTypeDef = () => {
    const typeDef = AnalyzedAssetTypeDef.extend({
      fields: {
    ...
    analyzedAssetTypeDefFields.value = typeDef.tabIndexedFields();
    analyzedAssetTypeDef.value = typeDef;
    tableFields.value = analyzedAssetTypeDef.value.tableFields && Array.isArray(analyzedAssetTypeDef.value.tableFields)
      ? analyzedAssetTypeDef.value.tableFields
      : analyzedAssetTypeDef.value.primary
        ? [analyzedAssetTypeDef.value.primary, "ctime", "mtime"]
        : ["id", "ctime", "mtime"];
  ...
  const onAddSubmitted = async (obj: MobilettoOrmObject) => {
    await analyzedAssetStore
      .create(obj as AnalyzedAssetType, createAnalyzedAssetServerErrors);
    addingObject.value = false;
    successSnackbar.value = parseMessage(props.msgAddSuccess, messages.value, {
        type: messages.value.typename_analyzedAsset,
        id: AnalyzedAssetTypeDef.id(obj),
    });
    emit("added", obj);
    addObject.value = {} as AnalyzedAssetType;
    return await analyzedAssetStore.search();
  }
  watch(createAnalyzedAssetServerErrors, () => {
    if (createAnalyzedAssetServerErrors.value && Object.keys(createAnalyzedAssetServerErrors.value).length > 0) {
      errorSnackbar.value = parseMessage(props.msgAddError, messages.value, {
        type: messages.value.typename_analyzedAsset,
        error: JSON.stringify(createAnalyzedAssetServerErrors),
      });
    }
  });
  ...
  const onEditSubmitted = async (obj: MobilettoOrmObject) => {
    await analyzedAssetStore
      .update(obj as AnalyzedAssetType, editAnalyzedAssetServerErrors);
    editingObject.value = {} as AnalyzedAssetType;
  ...
  watch(editAnalyzedAssetServerErrors, () => {
    if (editAnalyzedAssetServerErrors.value && Object.keys(editAnalyzedAssetServerErrors.value).length > 0) {
      errorSnackbar.value = parseMessage(props.msgEditError, messages.value, {
        type: messages.value.typename_analyzedAsset,
        id: AnalyzedAssetTypeDef.id(editingObject.value),
        error: JSON.stringify(editAnalyzedAssetServerErrors),
      });
    }
  });
  ...
  const showEditOrm = (obj: MobilettoOrmObject) => {
    ...
    if (obj) {
      const id = AnalyzedAssetTypeDef.id(obj);
      if (id && id.length > 0) {
        addingObject.value = false; // ensure add form is closed
        editingObject.value = JSON.parse(JSON.stringify(obj)) as AnalyzedAssetType;
      }
    }
  };
  const onEditCancel = () => {
  ...
  const delObject = (obj: MobilettoOrmObject) => {
      const id = AnalyzedAssetTypeDef.id(obj);
      analyzedAssetStore.delete(id, deleteAnalyzedAssetServerErrors)
          .then(() => {
              ...
              emit("deleted", obj);
              return analyzedAssetStore.search();
          });
  };
  watch(deleteAnalyzedAssetServerErrors, () => {
    if (deleteAnalyzedAssetServerErrors.value && Object.keys(deleteAnalyzedAssetServerErrors.value).length > 0) {
        errorSnackbar.value = parseMessage(props.msgDeleteError, messages.value, {
            type: messages.value.typename_analyzedAsset,
            id: AnalyzedAssetTypeDef.id(obj),
            error: JSON.stringify(deleteAnalyzedAssetServerErrors),
        });
    }
  });
  watch(analyzedAssetList, (newList) => {
    if (newList && Array.isArray(newList) && newList.length === 0 && searchTerms.value && searchTerms.value.length === 0) {
      if (navigating.value) return;
      navigating.value = true;
      navigateTo("/admin/analyzedAsset/setup");
    }
  });
  ...
  analyzedAssetStore.search().then(() => {
    initRefs();
  });
</script>
