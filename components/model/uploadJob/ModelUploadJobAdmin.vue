...
<template>
  <v-container>
    ...
    <v-row>
      <v-col>
        <h2>{{ adminTitle() }}</h2>
        <b v-if="messageExists('admin_title_uploadJob_administration_details', messages)">
          {{ messages.admin_title_uploadJob_administration_details }}
        </b>
      </v-col>
    </v-row>
  ...
  watch(deleteUploadJobServerErrors, () => {
    if (deleteUploadJobServerErrors.value && Object.keys(deleteUploadJobServerErrors.value).length > 0) {
        errorSnackbar.value = parseMessage(props.msgDeleteError, messages.value, {
            type: messages.value.typename_uploadJob,
            id: deletingObject.value ? UploadJobTypeDef.id(deletingObject.value) : "null",
