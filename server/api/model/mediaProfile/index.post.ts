  ...
  return await filterErrors(event, "mediaProfile.search", async (event: H3Event) => {
    return await requireAdminAccountObject(event, "mediaProfile.search", async (event: H3Event, session, account) => {
      ...
      if (typeof MediaProfileTypeDef.apiConfig.search?.validate === "function") {
        ...
        const validated = await MediaProfileTypeDef.apiConfig.search.validate(caller, null, opts);
      ...
      const predicate = opts.textSearch && opts.textSearch.trim().length > 0
        ? (obj: MobilettoOrmObject) => MediaProfileTypeDef.textMatch(obj, opts.textSearch || "")
        : undefined;
      ...
      if (opts.field && opts.value) {
        return await mediaProfileRepo.safeFindBy(opts.field, opts.value, searchOpts);
      } else {
        return await mediaProfileRepo.find(searchOpts);
      }
    });
  });
});
