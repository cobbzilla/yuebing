  ...
  return await filterErrors(event, "sourceAsset.search", async (event: H3Event) => {
    return await requireAdminAccountObject(event, "sourceAsset.search", async (event: H3Event, session, account) => {
      ...
      if (typeof SourceAssetTypeDef.apiConfig.search?.validate === "function") {
        ...
        const validated = await SourceAssetTypeDef.apiConfig.search.validate(caller, null, opts);
      ...
      const predicate = opts.textSearch && opts.textSearch.trim().length > 0
        ? (obj: MobilettoOrmObject) => SourceAssetTypeDef.textMatch(obj, opts.textSearch || "")
        : undefined;
