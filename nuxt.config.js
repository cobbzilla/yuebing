...
var EMAIL_ENABLED = typeof process.env.YB_EMAIL_HOST === 'string' && process.env.YB_EMAIL_HOST.length > 0;
var EMAIL_REQUIRED = EMAIL_ENABLED ? 'required|' : '';
var AUTOSCAN_INTERVAL_FIELD = {
...
exports.default = defineNuxtConfig({
    ...
    head: {
        ...
        link: [
            { rel: 'icon', type: 'image/x-icon', href: "".concat(process.env.YB_FAVICON_DIR || '', "/favicon.ico") },
            { rel: 'apple-touch-icon', sizes: '180x180', href: "".concat(process.env.YB_FAVICON_DIR || '', "/apple-touch-icon.png") },
            { rel: 'icon', type: 'image/png', sizes: '32x32', href: "".concat(process.env.YB_FAVICON_DIR || '', "/favicon-32x32.png") },
            { rel: 'icon', type: 'image/png', sizes: '16x16', href: "".concat(process.env.YB_FAVICON_DIR || '', "/favicon-16x16.png") },
            { rel: 'manifest', href: "".concat(process.env.YB_FAVICON_DIR || '', "/site.webmanifest") },
            { rel: 'mask-icon', href: "".concat(process.env.YB_FAVICON_DIR || '', "/safari-pinned-tab.svg"), color: '#5bbad5' }
        ]
    },
    runtimeConfig: {
        public: {
            ...
            timeout: {
                verify: process.env.YB_TIMEOUT_ACCOUNT_VERIFICATION || 1000 * 60 * 60 * 24 * 2,
                resetPassword: process.env.YB_TIMEOUT_RESET_PASSWORD || 1000 * 60 * 60,
        ...
        private: {
            ...
            email: {
                ...
                configurable: {
                    host: {
                        rules: "".concat(EMAIL_REQUIRED, "host|min:6|max:128")
                    },
                    port: {
                        rules: "".concat(EMAIL_REQUIRED, "integer|min_value:10|max_value:65000")
                    },
                    user: {
                        rules: "".concat(EMAIL_REQUIRED, "min:2|max:100")
                    },
                    password: {
                        rules: "".concat(EMAIL_REQUIRED, "min:2|max:100")
                    },
                    ...
                    fromEmail: {
                        rules: "".concat(EMAIL_REQUIRED, "email|min:2|max:100")
                    }
                }
            },
            ...
            redis: {
                ...
                manifestCacheExpiration: process.env.YB_MANIFEST_CACHE_EXPIRATION || 60 * 1000,
                configurable: {
                    host: {
            ...
            session: {
                ...
                expiration: process.env.YB_SESSION_EXPIRATION || 1000 * 60 * 60 * 24,
                configurable: {
                    expiration: {
                        rules: 'required|integer|min_value:0|max_value:3153600000000',
                        format: 'duration',
            ...
            autoscan: {
                ...
                interval: process.env.YB_AUTOSCAN_INTERVAL || 1000 * 60 * 60 * 24,
                // How long to wait before the initial startup scan
                // Minimum interval is 5 seconds. Lower settings are ignored.
                initialDelay: process.env.YB_AUTOSCAN_INITIAL_DELAY || 1000 * 60 * 2,
